<section class="service-area">
    <div class="container">
        <div class="row">
            <div class="col-xl-12 mb-80 d-inline-flex">
                <div class="col-lg-9 section-title">
                    <h1>Recent publications</h1>
                    <!-- <p>Kabupaten Bogor </p> -->
                </div>
            </div>
        </div>
    </div>
    <div class="container-fluid-cus sec-publication">
        <div class="slider-publication">

            <?php foreach ($param["data"] as $data) : ?>
                <?php include 'recent-publications-card.php' ?>
            <?php endforeach; ?>

            <div class="box-black non-images">
                <div class="hover-dw">
                    <div class="hov-title">
                        1st Design Factory Global Network Research Conference ‘Designing the Future’ 5-6 October 2022
                    </div>
                    <div class="hov-dwonload">
                        <div class="icwd">
                            <a href="https://research.aalto.fi/en/publications/1st-design-factory-global-network-research-conference-designing-t" target="_blank">
                                <img src="<?php echo get_template_directory_uri() . '/assets/img/assets/Download.png'; ?>" alt="">
                            </a>
                            <span>PDF</span>
                        </div>
                        <div class="year">
                            2022
                        </div>
                    </div>
                </div>
                <div class="lipet">
                    <svg xmlns="http://www.w3.org/2000/svg" width="110" height="102" viewBox="0 0 110 102" fill="none">
                        <path d="M0.222656 0V101.333H110L0.222656 0Z" fill="#384042" />
                        <!-- <path opacity="0.35" d="M0.222656 0.88916V114.889L110 85.3334L0.222656 0.88916Z" fill="black"/> -->
                    </svg>

                </div>
                <div class="lipet-shade">
                    <svg xmlns="http://www.w3.org/2000/svg" width="110" height="115" viewBox="0 0 110 115" fill="none">
                        <path opacity="0.35" d="M0.222656 0.88916V114.889L110 85.3334L0.222656 0.88916Z" fill="black" />
                    </svg>

                </div>
                <svg xmlns="http://www.w3.org/2000/svg" width="304" height="365" viewBox="0 0 304 365" fill="none">
                    <path d="M0 388.444V0H194.222L303.999 101.333V388.444H0Z" fill="#273032" />
                </svg>
            </div>

            <div class="box-black">
                <div class="hover-dw">
                    <div class="hov-title">
                        DFGN 10th Anniversary Cookbook
                    </div>
                    <div class="hov-dwonload">
                        <div class="icwd">
                            <a href="img/assets/pdf/DFGN_cookbook.pdf" target="_blank">
                                <img src="<?php echo get_template_directory_uri() . '/assets/img/assets/Download.png'; ?>" alt="">
                            </a>
                            <span>PDF</span>
                        </div>
                        <div class="year">
                            2021
                        </div>
                    </div>
                </div>
                <div class="lipet">
                    <svg xmlns="http://www.w3.org/2000/svg" width="110" height="102" viewBox="0 0 110 102" fill="none">
                        <path d="M0.222656 0V101.333H110L0.222656 0Z" fill="white" />
                        <!-- <path opacity="0.35" d="M0.222656 0.88916V114.889L110 85.3334L0.222656 0.88916Z" fill="black"/> -->
                    </svg>

                </div>
                <div class="lipet-shade">
                    <svg xmlns="http://www.w3.org/2000/svg" width="110" height="115" viewBox="0 0 110 115" fill="none">
                        <path opacity="0.35" d="M0.222656 0.88916V114.889L110 85.3334L0.222656 0.88916Z" fill="black" />
                    </svg>

                </div>
                <svg id="svg-pub">
                    <defs>
                        <mask id="mask-pub">
                            <path d="M0 388.444V0H194.222L303.999 101.333V388.444H0Z" fill="#FFFFFF" />
                        </mask>
                    </defs>
                </svg>

                <div class="load-img">
                    <img src="<?php echo get_template_directory_uri() . '/assets/img/assets/pub3.png'; ?>" alt="">
                </div>
            </div>

            <div class="box-black">
                <div class="hover-dw">
                    <div class="hov-title">
                        DFGN: We are not like the other design factories
                    </div>
                    <div class="hov-dwonload">
                        <div class="icwd">
                            <a href="https://issuu.com/aaltodesignfactory/docs/dfgn_issuu" target="_blank">
                                <img src="<?php echo get_template_directory_uri() . '/assets/img/assets/Download.png'; ?>" alt="">
                            </a>
                            <span>PDF</span>
                        </div>
                        <div class="year">
                            2020
                        </div>
                    </div>
                </div>
                <div class="lipet">
                    <svg xmlns="http://www.w3.org/2000/svg" width="110" height="102" viewBox="0 0 110 102" fill="none">
                        <path d="M0.222656 0V101.333H110L0.222656 0Z" fill="white" />
                        <!-- <path opacity="0.35" d="M0.222656 0.88916V114.889L110 85.3334L0.222656 0.88916Z" fill="black"/> -->
                    </svg>

                </div>
                <div class="lipet-shade">
                    <svg xmlns="http://www.w3.org/2000/svg" width="110" height="115" viewBox="0 0 110 115" fill="none">
                        <path opacity="0.35" d="M0.222656 0.88916V114.889L110 85.3334L0.222656 0.88916Z" fill="black" />
                    </svg>

                </div>
                <svg id="svg-pub">
                    <defs>
                        <mask id="mask-pub">
                            <path d="M0 388.444V0H194.222L303.999 101.333V388.444H0Z" fill="#FFFFFF" />
                        </mask>
                    </defs>
                </svg>

                <div class="load-img">
                    <img src="<?php echo get_template_directory_uri() . '/assets/img/assets/pub1.png'; ?>" alt="">
                </div>
            </div>

            <div class="box-black">
                <div class="hover-dw">
                    <div class="hov-title">
                        The Interstellar Family Album
                    </div>
                    <div class="hov-dwonload">
                        <div class="icwd">
                            <a href="img/assets/pdf/Interstellar_FamilyAlbum.pdf" target="_blank">
                                <img src="<?php echo get_template_directory_uri() . '/assets/img/assets/Download.png'; ?>" alt="">
                            </a>
                            <span>PDF</span>
                        </div>
                        <div class="year">
                            2019
                        </div>
                    </div>
                </div>
                <div class="lipet">
                    <svg xmlns="http://www.w3.org/2000/svg" width="110" height="102" viewBox="0 0 110 102" fill="none">
                        <path d="M0.222656 0V101.333H110L0.222656 0Z" fill="white" />
                        <!-- <path opacity="0.35" d="M0.222656 0.88916V114.889L110 85.3334L0.222656 0.88916Z" fill="black"/> -->
                    </svg>

                </div>
                <div class="lipet-shade">
                    <svg xmlns="http://www.w3.org/2000/svg" width="110" height="115" viewBox="0 0 110 115" fill="none">
                        <path opacity="0.35" d="M0.222656 0.88916V114.889L110 85.3334L0.222656 0.88916Z" fill="black" />
                    </svg>

                </div>
                <svg id="svg-pub">
                    <defs>
                        <mask id="mask-pub">
                            <path d="M0 388.444V0H194.222L303.999 101.333V388.444H0Z" fill="#FFFFFF" />
                        </mask>
                    </defs>
                </svg>

                <div class="load-img">
                    <img src="<?php echo get_template_directory_uri() . '/assets/img/assets/pub2.png'; ?>" alt="">
                </div>

            </div>


        </div>
    </div>
